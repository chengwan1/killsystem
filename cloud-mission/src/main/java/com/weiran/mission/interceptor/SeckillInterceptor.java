...
//            String requestURI = request.getRequestURI();
            String visitorId = request.getHeader("X-Visitor-ID"); // 从请求头中获取设备ID
            AccessKey accessKey = AccessKey.withExpire;
            // 把redis对应access的key前缀和请求url和整数int的类信息去redis取出相应的对象
            Integer count = redisService.get(accessKey, visitorId, Integer.class);
            // 这里是，对于打上限流注解的方法，当限制小于maxCount时，redis里的对应的key的value值加1，如果大于等于maxCount，则由这里的拦截器直接返回访问繁忙信息，进行拦截。
            if (count  == null) {
                redisService.set(accessKey, visitorId, 1, seconds);
            } else if (count < maxCount) {
                redisService.increase(accessKey, visitorId);
            } else {
                log.info("访问频繁,请稍后重试");
                render(response);
                return false;
